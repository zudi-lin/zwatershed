...
template< typename ID, typename F, typename L, typename H > 
inline void steepestascent(
    boost::multi_array_ref<F, 4>& aff, 
    ...
    std::cout << "Steepest ascent: volume = " << xdim << ", " << ydim << ", " <<zdim << std::endl;
    std::cout << "                 seg vol = " << seg.shape()[0] << ", " << seg.shape()[1] << ", " << seg.shape()[2] << std::endl;
...
template< typename ID> inline void divideplateaus(
    ...
    for ( std::ptrdiff_t idx = 0; idx < size; ++idx )
    {
        for ( std::ptrdiff_t d = 0; d < 6; ++d )
        {
            ...
            {
                if (idx + dir[d] < 0) {
                    std::cout << "UNDERFLOW: idx="<< idx << " d=" << d << " dir[d]=" << dir[d] << std::endl;
                    return;
                }
                if (idx + dir[d] >= size) {
                    std::cout << "OVERFLOW: idx="<< idx << " d=" << d << " dir[d]=" << dir[d] << std::endl;
                    return;
                }
                if ( !(seg_raw[idx+dir[d]] & idirmask[d]) )
                {
    ...
    std::cout << "divide plateaus" << std::endl;
    // divide the plateaus
...
template< typename ID, typename SIZE_T> inline void findbasins(
    boost::multi_array_ref<ID, 3> seg, std::vector<SIZE_T> &counts) {
    using id_t       = ID;
    using traits     = watershed_traits<id_t>;
